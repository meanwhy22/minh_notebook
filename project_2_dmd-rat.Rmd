---
title: "Single-cell RNA-seq analysis of DMD rat model"
bibliography: projects/project_2/references.bib
nocite: '@*'
output:
  html_document:
...
> **Date:** 18/10/2025
>
> **Description:** [Taglietti et al. (2022)](https://doi.org/10.1186/s40478-022-01355-2) created a rat model for DMD by CRISPR KO of exon 52 of dystrophin-encoding gene in the animal zygotes. Following functional assessment, they performed the single-cell RNA-seq of diaphragms of healthy (**WT**) and **R-DMDdel52** 12-month-old rats. In this practice session, we'll start from the count matrix deposited in NCBI Gene Expression Omnibus database under accession GSE198237.
...
<summary>**Create Seurat objects**</summary>

```{r read10X, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r percent.mt, echo=TRUE, message=FALSE, warning=FALSE}
samples <- c("DIA_WT_12months", "DIA_DMD_12months")
...
```{r view_metadata, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r save_QC_plots, eval=FALSE, include=FALSE}
# make output folder for plots
dir.create("projects/project_2/QC_plots", showWarnings = FALSE)
...
for (obj_name in samples) {
  ...
  # save to file
  ggsave(
    filename = paste0("projects/project_2/QC_plots/", obj_name, "_QCplots.pdf"),
    plot = combined_plot,
    width = 12, height = 10
  )
}
...
<summary>**QC plots before filtering**</summary>
...
```{r WT_filtering, echo=TRUE, message=FALSE, warning=FALSE}
# Filtering ----
DIA_WT_subset <- subset(DIA_WT_12months, subset = nFeature_RNA > 300 & nFeature_RNA < 4000 & nCount_RNA < 15000 & percent.mt < 25)
```

```{r WT_QC_plots_filtered, echo=FALSE, fig.height=10, fig.width=12, message=FALSE, warning=FALSE}
# WT
...
```{r DMD_filtering, echo=TRUE, message=FALSE, warning=FALSE}
# Filtering ----
DIA_DMD_subset <- subset(DIA_DMD_12months, subset = nFeature_RNA > 300 & nFeature_RNA < 4000 & nCount_RNA > 200 & nCount_RNA < 15000 & percent.mt < 25)
```

```{r DMD_QC_plots_filtered, echo=FALSE, fig.height=10, fig.width=12, message=FALSE, warning=FALSE}
# DMD
...
<summary>**Merge Seurat objects**</summary>

```{r merge, echo=TRUE, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
...
```{r merged_data, echo=FALSE, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
merged_data
p1 / (p2 | p3)
```
...
```{r seurat_workflow, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r elbow_plot_pca, echo=FALSE, fig.height=3, fig.width=4, message=FALSE, warning=FALSE}
ElbowPlot(merged_data, reduction = 'RNA_pca', ndims = 50)
```
...
```{r cumulative_var, echo=TRUE, message=FALSE, warning=FALSE}
...
<summary>**`FindAllMarkers`**</summary>
...
knitr::kable(head(top_markers), caption = "Top marker genes in each cluster") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```

```{r do_heatmap_top_markers, echo=FALSE, fig.height=12, fig.width=10, message=FALSE, warning=FALSE}
...
```{r classical_markers, echo=FALSE, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
classical_markers <- c(
  "Cdh5",                              # endothelial
...
<summary>Assign new cell cluster identities</summary>

```{r annotation, echo=TRUE, message=FALSE, warning=FALSE}
# assign cell ids ----
new.cluster.ids <- c(   
  "0" = "Endothelial",   
  "1" = "Endothelial",   
...
<summary>**GO analysis & visualization**</summary>

```{r go_enrichment, echo=TRUE, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
merged_data <- readRDS("~/project_2_dmd-rat/merged_data.rds")
...
```{r go_plots_all_clusters, echo=FALSE, fig.height=8, fig.width=10}
...
```{r cell_type_composition, echo=FALSE, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
# Cell type composition ----
...
```{r DMD_vs_WT, echo=TRUE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
...
```{r DMD_vs_WT_volcano_plot, echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
...
```{r go_enrich_among_DEGs, echo=FALSE, fig.height=5, fig.width=18}
# Combine plots ----
...
<summary>**Step 3: Run GSEA with all GOBP gene sets**</summary>
...
<summary>**Visualize GSEA results: Up & Down GOBPs**</summary>
...
(also known as Co-expression Network Analysis; though we don't seem to have enough samples for this task.) This part follows the [tutorial](https://smorabit.github.io/hdWGCNA/articles/basic_tutorial.html) on hdWGCNA package website.
...
```{r metacells, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r cor.test_volcano_plot, echo=FALSE, fig.height=5, fig.width=5, warning=FALSE}
