...
```{r QC_metrics, eval=FALSE, include=FALSE}
# make output folder for plots
dir.create("QC_plots_filtered", showWarnings = FALSE)

# loop the visualization of QC metrics ----
samples_subset <- c("wtNSG01_subset", "wtNSG02_subset", "mdxNSG01_subset", "mdxNSG02_subset", "mdxD2NSG01_subset", "mdxD2NSG02_subset")
...
```{r QC_plots, fig.height=10, fig.width=12}
# example plot
...
```{r filtered_QC, fig.height=5, fig.width=18}
...
# sum(rownames(wtNSG01) %in% s.genes)
```

```{r cell_cycle_scoring, fig.height=8, fig.width=12}
...
```{r cell_cycle_pca, fig.height=5, fig.width=12}
...
| [Note:]{.underline} After running, SCT becomes the active assay. Different numbers of highly variable genes (HVGs) were returned by two methods.
...
```{r PCA_Elbow_plots, fig.height=12, fig.width=8}
...
```{r unintegrated, fig.height=3, fig.width=4}
...
```{r cluster_before_integrate, fig.height=4, fig.width=15}
...
```{r harmony, fig.height=3, fig.width=4}
...
```{r umap_plot_after_integration, fig.height=4, fig.width=15}
...
Look at the markers indentified in the paper:

```{r markers_paper, fig.height=15, fig.width=12}
...
Continue using a previously saved Seurat object:

```{r cellnumber_condition}
...
```{r cell_type_composition, fig.height=10, fig.width=15}
# Cell type composition ----
...
Since previously we combined 2 clusters into 'Stromal', let's do `FindAllMarkers` again:

```{r find_markers_again}
...
```{r dotplot, fig.height=15, fig.width=7}
...
```{r clustered_dotplot, fig.height=12, fig.width=5}
...
```{r heatmap, fig.height=18, fig.width=15}
...
`DoHeatmap` does not support hierarchical clustering; if needed, we can take the scaled expression table and do it manually.

| [Note:]{.underline} `DoHeatmap(integrated_harmony, features = marker_gene_list)$data` is a scaled matrix.

***Can we do a heatmap of average expression of each feature across all cells of a cell type?***

```{r heatmap_avg_expr, fig.height=10, fig.width=5}
...
Get expression matrix from Seurat object using a function adopted from [chatomics](https://divingintogeneticsandgenomics.com/post/how-to-make-a-multi-group-dotplot-for-single-cell-rnaseq-data/):

```{r multi-cluster.condition_function}
# create a function ----
...
mat <- GetMatrixFromSeuratByGroupSingle(obj = integrated_harmony, feature = "Adamtsl2", celltype, condition)
```
...
```{r multi-cluster.condition_1_gene, fig.height=5, fig.width=10}
...
```{r add_percentage_legend, fig.height=5, fig.width=12}
lgd_list = list(
...
```{r stromal_subcluster, fig.height=5, fig.width=15}
...
```{r stromal_subcluster_harmony, fig.height=10, fig.width=15}
...
```{r stromal_find_subcluster, fig.height=10, fig.width=15}
...
```{r MuSC_subset, fig.height=4, fig.width=12}
...
DimPlot(MuSC_sub, reduction = "RNA_umap", group.by = "orig.ident", split.by = "condition")
```

```{r MuSC_subset_harmony, fig.height=8, fig.width=12}
# RunHarmony again
...
```{r MuSC_find_subcluster, fig.height=10, fig.width=15}
# sub-cluster MuSCs ----
...
Chatomics (check this [post](https://divingintogeneticsandgenomics.com/post/how-to-make-a-multi-group-dotplot-for-single-cell-rnaseq-data/) on how to make a multi-group dotplot)
